//<!--
// \include\user_behavioral_tracking\PulsarCookieJar.js 

function _plsUBTCookiesObj(){var Default_Cookie_Format={"COOKIELET_DELIMITER":"^","NAME_VALUE_DELIMITER":"/","escapedValue":true},DP_Cookie_Format={"COOKIELET_DELIMITER":"^","NAME_VALUE_DELIMITER":"/","bUseExp":true,"startDelim":"b"},Session_Cookie_Format={"COOKIELET_DELIMITER":"^","NAME_VALUE_DELIMITER":"=","escapedValue":true,"startDelim":"^"},DS_Cookie_Format={"COOKIELET_DELIMITER":"^","NAME_VALUE_DELIMITER":"/"},sPath="/",aConversionMap={'reg':['dp1','reg'],'recent_vi':['ebay','lvmn'],'ebaysignin':['ebay','sin'],'p':['dp1','p'],'etfc':['dp1','etfc'],'keepmesignin':['dp1','kms'],'ItemList':['ebay','wl'],'BackToList':['s','BIBO_BACK_TO_LIST']},aFormatMap={'r':Default_Cookie_Format,'dp1':DP_Cookie_Format,'npii':DP_Cookie_Format,'ebay':Session_Cookie_Format,'reg':Session_Cookie_Format,'apcCookies':Session_Cookie_Format,'ds2':DS_Cookie_Format},sCOMPAT="10",sCONVER="01",sSTRICT="00",sModesCookie="ebay",sModesCookielet="cv";return{readCookie:function(psCookie,psCookielet){var rv=this.readCookieObj(psCookie,psCookielet).value;return(rv)?decodeURIComponent(rv):"";},createDefaultCookieBean:function(psCookie,psCookielet){var cookie={};cookie.name=psCookie;cookie.cookieletname=psCookielet;cookie.value="";cookie.maxage=0;cookie.rawcookievalue="";cookie.mode="";return cookie;},readCookieObj:function(psCookie,psCookielet){var cookie=this.createDefaultCookieBean(psCookie,psCookielet);this.update();this.checkConversionMap(cookie);cookie.rawcookievalue=this.aCookies[cookie.name];if(!cookie.name||!cookie.rawcookievalue){cookie.value="";}
else if(!cookie.cookieletname){this.readCookieInternal(cookie);}
else{this.readCookieletInternal(cookie);}
var guid=(psCookielet&&psCookielet.match(/guid$/));var object=(typeof(cookie)!='undefined')?cookie:'';var corrupted=(object&&guid&&(cookie.value.length>32));if(corrupted)cookie.value=cookie.value.substring(0,32);return object;},checkConversionMap:function(cookie){var cmap=aConversionMap[cookie.name];if(cmap){cookie.mode=this.getMode(cookie.name);cookie.name=cmap[0];cookie.cookieletname=cmap[1];}},readCookieInternal:function(cookie){cookie.value=cookie.rawcookievalue;return cookie;},readCookieletInternal:function(cookie){var clet=this.getCookielet(cookie.name,cookie.cookieletname,cookie.rawcookievalue);var format=this.getFormat(cookie.name);if(clet&&format.bUseExp){var cletOrig=clet;clet=clet.substring(0,clet.length-8);if(cletOrig.length>8){cookie.maxage=cletOrig.substring(cletOrig.length-8);}}
cookie.value=clet;if(cookie.mode==sCOMPAT){cookie.value=cookie.rawcookievalue;}
return cookie;},readMultiLineCookie:function(psCookie,psCookielet){if(!psCookie||!psCookielet){return"";}
var val,r="";var cmap=aConversionMap[psCookie];if(cmap){val=this.readCookieObj(cmap[0],cmap[1]).value||"";}
if(val){r=this.getCookielet(psCookie,psCookielet,val)||"";}
return(typeof(r)!="undefined")?r:"";},writeCookie:function(psCookie,psVal,psExp){var cmap=aConversionMap[psCookie];if(cmap){this.writeCookielet(cmap[0],cmap[1],psVal,psExp);return;}
var format=this.getFormat(psCookie);if(psVal&&format.escapedValue){psVal=encodeURIComponent(psVal);}
this.writeRawCookie(psCookie,psVal,psExp);},writeRawCookie:function(psCookie,psVal,psExp){if(psCookie&&(psVal!==undefined)){if((isNaN(psVal)&&psVal.length<4000)||(psVal+'').length<4000){if(typeof psExp=='number'){psExp=this.getExpDate(psExp);}
var expDate=psExp?new Date(psExp):new Date(this.getExpDate(730));var format=this.getFormat(psCookie);var sHost=this.sCookieDomain;var dd=document.domain;if(dd.indexOf(sHost)==-1){var index=dd.indexOf('.ebay.');if(index>0){this.sCookieDomain=dd.substring(index);}}
if(document.cookie)
{document.cookie=psCookie+"="+(psVal||"")+
((psExp||format.bUseExp)?"; expires="+expDate.toGMTString():"")+"; domain="+this.sCookieDomain+"; path="+sPath;}}}},writeCookieEx:function(psCookie,psVal,piDays){this.writeCookie(psCookie,psVal,this.getExpDate(piDays));},writeCookielet:function(psCookie,psCookielet,psVal,psExp,psContExp){if(psCookie&&psCookielet){this.update();var format=this.getFormat(psCookie);if(format.bUseExp&&psVal){if(typeof psExp=='number'){psExp=this.getExpDate(psExp);}
var expDate=psExp?new Date(psExp):new Date(this.getExpDate(730));var expDateUTC=Date.UTC(expDate.getUTCFullYear(),expDate.getUTCMonth(),expDate.getUTCDate(),expDate.getUTCHours(),expDate.getUTCMinutes(),expDate.getUTCSeconds());expDateUTC=Math.floor(expDateUTC/1000);psVal+=parseInt(expDateUTC,10).toString(16);}
var val=this.createCookieValue(psCookie,psCookielet,psVal);this.writeRawCookie(psCookie,val,psContExp);}},writeMultiLineCookie:function(psCookie,psCookielet,psVal,psExp,psContExp){this.update();var val=this.createCookieValue(psCookie,psCookielet,psVal);if(val){var cmap=aConversionMap[psCookie];if(cmap){this.writeCookielet(cmap[0],cmap[1],val,psExp,psContExp);}}},getBitFlagOldVersion:function(piDec,piPos){var dec=parseInt(piDec,10);var b=dec.toString(2),r=dec?b.charAt(b.length-piPos-1):"";return(r=="1")?1:0;},setBitFlagOldVersion:function(piDec,piPos,piVal){var b="",p,i,e,l;piDec=parseInt(piDec+"",10);if(piDec)
{b=piDec.toString(2);}
l=b.length;if(l<piPos)
{e=piPos-l;for(i=0;i<=e;i++)
{b="0"+b;}}
p=b.length-piPos-1;return parseInt(b.substring(0,p)+piVal+b.substring(p+1),2);},getBitFlag:function(piDec,piPos){if(piDec!=null&&piDec.length>0&&piDec.charAt(0)=='#')
...
{return this.getBitFlagOldVersion(piDec,piPos);}},setBitFlag:function(piDec,piPos,piVal){if(piDec!=null&&piDec.length>0&&piDec.charAt(0)=='#')
...
{if(piPos>31)
{return piDec;}
return this.setBitFlagOldVersion(piDec,piPos,piVal);}},createCookieValue:function(psName,psKey,psVal){var cmap=aConversionMap[psName],format=this.getFormat(psName),mode=this.getMode(psName),val;if(cmap&&(mode==sSTRICT||mode==sCONVER)){val=this.readCookieObj(cmap[0],cmap[1]).value||"";}
else{val=this.aCookies[psName]||"";}
if(format){var clts=this.getCookieletArray(val,format);clts[psKey]=psVal;var str="";for(var i in clts){if(clts.hasOwnProperty(i)){str+=i+format.NAME_VALUE_DELIMITER+clts[i]+format.COOKIELET_DELIMITER;}}
if(str&&format.startDelim){str=format.startDelim+str;}
val=str;if(format.escapedValue){val=encodeURIComponent(val);}}
return val;},update:function(){var aC=document.cookie.split("; ");this.aCookies={};var regE=new RegExp('^"(.*)"$');for(var i=0;i<aC.length;i++){var sC=aC[i].split("=");var format=this.getFormat(sC[0]),cv=sC[1],sd=format.startDelim;if(sd&&cv&&cv.indexOf(sd)===0){sC[1]=cv.substring(sd.length,cv.length);}
if(sC[1]&&sC[1].match(regE)){sC[1]=sC[1].substring(1,sC[1].length-1);}
this.aCookies[sC[0]]=sC[1];}},getCookielet:function(psCookie,psCookielet,psVal){var format=this.getFormat(psCookie);var clts=this.getCookieletArray(psVal,format);return clts[psCookielet]||"";},getFormat:function(psCookie){return aFormatMap[psCookie]||Default_Cookie_Format;},getCookieletArray:function(psVal,poFormat){var rv=[],val=psVal||"";if(poFormat.escapedValue){val=decodeURIComponent(val);}
var a=val.split(poFormat.COOKIELET_DELIMITER);for(var i=0;i<a.length;i++){var idx=a[i].indexOf(poFormat.NAME_VALUE_DELIMITER);if(idx>0){rv[a[i].substring(0,idx)]=a[i].substring(idx+1);}}
return rv;},getExpDate:function(piDays){var expires;if(typeof piDays=="number"&&piDays>=0){var d=new Date();d.setTime(d.getTime()+(piDays*24*60*60*1000));expires=d.toGMTString();}
return expires;},getMode:function(psCookie){var h=this.readCookieObj(sModesCookie,sModesCookielet).value,b,i;if(!(psCookie in aConversionMap)){return null;}
if(!h){return"";}
if(h===0){return sSTRICT;}
if(h&&h!="0"){if(h.indexOf(".")!=-1){var a=h.split(".");for(var i=0;i<a.length;i++){b=parseInt(a[i],16).toString(2)+b;}}
else{b=parseInt(h,16).toString(2);}
i=0;var l=b.length,j;for(var o in aConversionMap)
{j=l-(2*(i+1));var f=b.substring(j,j+2).toString(10);f=(!f)?sSTRICT:f;if(psCookie==o)
{return(f.length==1)?"0"+f:f;}
i++;}
return null;}
return null;},getMulti:function(piDec,piPos,piBits){var r="",i,CJ=this;for(i=0;i<piBits;i++){r=CJ.getBitFlag(piDec,piPos+i)+r;}
return parseInt(r,2);},setMulti:function(piDec,piPos,piBits,piVal){var i=0,CJ=this,v,l,e;v=piVal.toString(2).substring(0,piBits);l=v.length;if(l<piBits){e=piBits-l;for(var j=0;j<e;j++){v="0"+v;}
l=l+e;}
for(i=0;i<l;i++){piDec=CJ.setBitFlag(piDec,piPos+i,v.substring(l-i-1,l-i));}
return piDec;},setJsCookie:function(){this.writeCookielet('ebay','js','1');}};};if(typeof raptor!=='undefined'){raptor.alias(require('ebay/cookies'),'raptor.dom.Cookie');}
...
if(inp.https!=null&&inp.https==true){baseURLTemplate="https://"+serverHost;}else{baseURLTemplate=protocol+"//"+serverHost;}}
...
function firePulsarProxyURL(unload){_plsUBTPPURL=_plsubtInp.URLTemplate+"pld=";var paylaod="[";plsUBTDebug("Initial event queue size: "+_plsUBTpld.length);for(var i=0;i<_plsUBTpld.length;i++){_plsUBTpld[i].difTS=new Date().getTime()-_plsUBTpld[i].difTS;paylaod=paylaod+JSON.stringify(_plsUBTpld[i]);if(i!=_plsUBTpld.length-1){paylaod=paylaod+", ";}};paylaod=paylaod+"]";_plsUBTPPURL=_plsUBTPPURL+encodeURIComponent(paylaod);plsUBTDebug("Call URL: "+_plsUBTPPURL);var browserInfo=getBrowserNameAndVersion();if(navigator.sendBeacon){if(browserInfo.name.toUpperCase()=='SAFARI'){if(browserInfo.version>=11){sendBeacon(_plsUBTPPURL);}else{plsUBTAjaxCall(unload);}}else{sendBeacon(_plsUBTPPURL);}}else{plsUBTAjaxCall(unload);}};function getBrowserNameAndVersion(){var ua=navigator.userAgent,tem,M=ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)||[];if(/trident/i.test(M[1])){tem=/\brv[:]+(\d+)/g.exec(ua)||[];return{name:'IE',version:(tem[1]||'')};}
if(M[1]==='Chrome'){tem=ua.match(/\bOPR|Edge\/(\d+)/)
if(tem!=null){return{name:'Opera',version:tem[1]};}}
M=M[2]?[M[1],M[2]]:[navigator.appName,navigator.appVersion,'-?'];if((tem=ua.match(/version\/(\d+)/i))!=null){M.splice(1,1,tem[1]);}
return{name:M[0],version:M[1]};}
firePulsarProxyURLAsImg=function(){for(var i=0;i<_plsUBTpld.length;i++){_plsUBTpld[i].difTS=new Date().getTime()-_plsUBTpld[i].difTS;var ppImpURL=_plsubtInp.URLTemplate+"pld="+encodeURIComponent("["+JSON.stringify(_plsUBTpld[i])+"]");var img=new Image();img.src=ppImpURL;}}
